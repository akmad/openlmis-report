        ...
        stage('Build') {
            steps {
                withCredentials([file(credentialsId: '8da5ba56-8ebb-4a6a-bdb5-43c9d0efb120', variable: 'ENV_FILE')]) {
                    ...
                    sh "docker tag openlmis/report:latest openlmis/report:${STAGING_VERSION}"
                }
            }
            post {
                success {
        ...
        stage('Parallel: Sonar analysis and contract tests') {
            parallel {
                ...
                stage('Contract tests') {
                    steps {
                        build job: 'OpenLMIS-report-contract-test', propagate: true, wait: true
                    }
                    post {
                        failure {
                            slackSend color: 'danger', message: "${env.JOB_NAME} - #${env.BUILD_NUMBER} ${env.STAGE_NAME} FAILED (<${env.BUILD_URL}|Open>)"
            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network erd_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s report -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
